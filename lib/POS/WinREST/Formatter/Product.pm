package POS::WinREST::Formatter::Product;
...
has record_definition => (
  ...
  default => sub {
    return [
      ...
      { name => 'order_code', size => 5, type => 'Numeric' },
      { name => 'halfdose_code', size => 6, type => 'Numeric' },
      { name => 'family_code', size => 4, type => 'Numeric' },
      { name => 'name', size => 40, type => 'Str' },
      { name => 'short_name', size => 13, type => 'Str' },
      { name => 'button_name', size => 35, type => 'Str' },
      { name => 'product_size', size => 1, type => 'Numeric', default => 0 },
